...
1. In the most likely scenario you do not have access to this, so you need to request the following from the customer's IT department. (If you do have access, either through a personal test project or for some other reason, you can create them yourself!)
    - Create 1 [app registration](https://learn.microsoft.com/en-us/power-apps/developer/data-platform/walkthrough-register-app-azure-active-directory) for the deployment [service principal](https://learn.microsoft.com/en-us/azure/active-directory/develop/app-objects-and-service-principals)
    - **[optional]** If you need your functions to be run on schedule(s): Create 1 additional app registration for the schedules service principal
    - Create a client secret for both the deployment and schedules service principals. Store them immediately in LastPass or similar password management tool together with the corresponding client ID for the app registration. You'll find that under Overview / Application (client) ID.
    - Add the deployment service principal as a member to an existing (or create a new) ADD security group.
    - Link the deployment service principal to a CDF group with capabilities necessary for functions deployment: copy the `Object ID` of the ADD group and use that as the `Source ID` for the CDF group. The latest updated deployment requirements can be found on [Confluence / Cognite Functions](https://cognitedata.atlassian.net/wiki/spaces/MLOP/pages/1963098642/Cognite+Functions), but **typically require the following**:
